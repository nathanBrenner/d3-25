(async function depGraph() {
  ...
  //     mockDependancies.dependencies.reduce((agg, dep) => {
  //       if (agg[dep.org]) {
  //         agg[dep.org].children = [...agg[dep.org].children, dep];
  //       } else {
  //         agg[dep.org] = { name: dep.org, parent: null, children: [dep] };
  //       }
  //       return agg;
  //     }, {})
  //   ).map(({ name, children, parent }) => {
  //     return {
  //       name,
  //       parent,
  //       children: Object.values(
  //         children.reduce((a, b) => {
  //           if (a[b.name]) {
  //             a[b.name].children = [...a[b.name].children, b];
  //           } else {
  //             a[b.name] = { name: b.name, parent: name, children: [b] };
  //           }
  //           return a;
  //         }, {})
  //       ),
  ...
  const treeData = await d3.json("foo.json").then((res) => ({
